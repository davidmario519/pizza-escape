...
// 쌓이는 피자박스 에셋(box-최종.png, 239x25 납작한 슬랩, 프레임을 꽉 채움)
const BOX_RATIO = 239 / 25;   // 종횡비 ≈ 9.56

// 문 에셋 콘텐츠 bbox — 새어나오는 '빛'(반투명 빔)까지 안 잘리게 alpha>4 전체 영역을 사용.
// (문 3004x5060 안에서 측정: x[855..2231] y[753..4191]. 좌하단 빔이 x=855까지 퍼짐)
const DOOR_SRC = [855, 753, 1377, 3439];   // sx, sy, sw, sh
const DOOR_RATIO = 1377 / 3439;            // 콘텐츠 종횡비(빛 포함)
// 문 '왼쪽 하단' 모서리 = 뒷벽/바닥 경계 기준 (좌하단 (1056,2845) → 새 bbox 기준 y비율)
const DOOR_BL_FY = (2845 - 753) / 3439;    // ≈ 0.608

// --- 방 꾸미기 오브제 (src/게임 오브젝트/오브제/{상,중,하}) ---
// 상=벽에 거는 것(시계·초상화·전단지…), 중=바닥에 세우는 가구(장롱·책상·서랍…), 하=바닥에 널브러진 것(박스·술병…).
// 매 플레이마다 몇 개를 랜덤으로 골라 방을 다르게 연출 (아파트뷰 이웃방도 동일 시스템 사용).
// 각 항목 h = 그릴 높이(디자인 단위, ×S). 폭은 PNG 종횡비로 자동. 모두 투명배경·바닥기준 크롭본.
const DECOR_DIR = { high: '상', mid: '중', low: '하' };
const DECOR_DEFS = {
  high: [   // 벽 (ROOM_TOP~seamY 사이 벽면에 걺)
    { file: '시계.png',      h: 46 },
    { file: '달력.png',      h: 52 },
    { file: '초상화.png',    h: 52 },
    { file: '자연화.png',    h: 46 },
  ...
  low: [    // 바닥 잡동사니 (캐릭터보다 뒤에 깔아 가독성 유지)
    { file: '박스.png',       h: 60 },
    { file: '술병.png',       h: 40 },
    { file: '쓰레기 봉투.png', h: 50 },
    { file: '중고책.png',     h: 52 },
  ],
};
...
const ZOOM_MS = 620;               // 줌아웃(메인→아파트)·줌인(아파트→메인) 전환 길이
...
const LOOK_MESSAGES = [
  '알고 보면 모두 조금씩 방에 붙잡혀 있어.\n멈춘 만큼 더 천천히 가면 돼.',
  '방에 너무 익숙해 졌나..\n게임은 끝나지 않았어. 리플레이!',
  '혼자인줄 알았는데, 다들 비슷한 짐을 지고 있었어.\n 나도 다시 한번!',
  '짐은 그대로지만, 혼자가 아니야.\n 조금만 더 해보자.',
  '아무것도 안 한 줄 알았는데, 사실 버티고 있었다\n다시 한번 해볼래?'
];
...
let roomDecor = [];                 // 현재 메인 방의 오브제 배치 (resetGame에서 랜덤 생성, 아파트뷰는 방별로 교체)
...
let zoomStart = 0;                 // 줌인(아파트→메인 복귀) 애니메이션 시작 시각
...
function preload() {
  ...
  IMAGES = {
    ...
    // 게임 오브젝트 에셋 (디자인 PNG)
    box:  loadImage('src/게임 오브젝트/box-최종.png'),                          // 쌓이는 피자박스 (239x25 납작 슬랩)
    ...
    // 메인 플레이 조작 버튼 (투명 배경, 텍스트 포함)
    btnForward: loadImage('src/버튼/앞으로가기버튼.png'),   // 피치 '앞으로 가기' (콘텐츠 bbox 40,94 2273x1296)
    btnRest:    loadImage('src/버튼/잠시쉬기버튼 _4.png'),  // 녹색 '잠시 쉬기' (콘텐츠 bbox 14,74 1364x713)
  ...
  // 방 꾸미기 오브제 — 상/중/하 폴더에서 각각 로드 (DECOR_DEFS 순서대로 {img, h})
  IMAGES.decor = { high: [], mid: [], low: [] };
  for (const tier of ['high', 'mid', 'low']) {
    IMAGES.decor[tier] = DECOR_DEFS[tier].map(d => ({
      img: loadImage('src/게임 오브젝트/오브제/' + DECOR_DIR[tier] + '/' + d.file),
      h: d.h,
    }));
  }
}
...
function resetGame() {
  ...
  apartmentRooms = [];
  titleSlide = 0;
  roomDecor = makeRoomDecor();   // 이번 플레이의 방 꾸미기 (랜덤) — 매 판 다른 방
...
function renderFrame() {
  ...
  if (phase === 'lookaround') { drawLookaround(); return; }  // 줌아웃 → 손잡기 → 아파트 + 멘트 → 되돌아가기
  if (phase === 'zoomin')     { drawZoomIn();    return; }   // 되돌아가기 → 내 방으로 줌인 → 메인 플레이 복귀
...
// 둘러보기 진입 연출: (0) 줌아웃(메인→내 방 창) → (1) 손잡기 인트로(연대) → (2) 페이드아웃 → (3) 멘트 → 되돌아가기.
// 손잡기 PNG는 손은 불투명·배경은 반투명 어두운 워시라, 아파트 위에 얹으면 아파트가 살짝 어두워지며 손이 도드라진다.
function drawLookaround() {
  const e = millis() - lookStart;

  // (0) 줌아웃 — 메인 플레이가 작아지며 아파트의 '내 방' 창으로 들어간다.
  if (e < ZOOM_MS) {
    drawApartment();                            // 뒤에서 드러나는 아파트
    const te = easeInOut(e / ZOOM_MS);
    const c = apartmentCellRect(0);             // 내 방 창
    drawMiniPlayScene(lerp(0, c.x, te), lerp(0, c.y, te), lerp(W, c.w, te), lerp(H, c.h, te), liveRoom());
    return;
  }

  const e2 = e - ZOOM_MS;                        // 줌아웃 이후 타임라인
  drawApartment();                              // 손잡기의 배경 컨텍스트 = 줌아웃된 아파트
  if (e2 < HAND_MS) {
    const eh = e2 - HAND_F0;                    // 손 등장 이후 경과
    if (eh < 0) return;                         // 공백 프레임: 아파트만 (손은 아직 없음)
    let a = 255;
    if (eh < HAND_APPEAR) a = map(eh, 0, HAND_APPEAR, 0, 255);                 // 손 자연스럽게 등장(페이드인)
    if (e2 > HAND_MS - HAND_FADE) a = Math.min(a, map(e2, HAND_MS - HAND_FADE, HAND_MS, 255, 0));  // 끝에 페이드아웃
...
function drawScene() {
  ...
  // 문 배치 — 우측 가장자리에서 살짝 들여 문 슬랩·빛이 화면 밖으로 잘리지 않게 한다.
  const doorTopY = ROOM_TOP + 4 * S;
  const doorH = floorY - doorTopY;
  const doorW = doorH * DOOR_RATIO;              // 문 에셋 콘텐츠 종횡비(빛 포함)
  const doorCx = W * 0.978 - doorW / 2;          // 콘텐츠 우측 가장자리가 화면 0.978W 안쪽에 오도록
  const seamY = doorTopY + DOOR_BL_FY * doorH;   // 뒷벽/바닥 경계 = 문 왼쪽 하단 높이
                                                 // (문은 이 선에 발치 왼쪽이 닿고, 베이스는 전경으로 비스듬히 내려옴)

  drawRoomBackground(seamY, b);              // 네이비 뒷벽 + 바닥 (균일 밝기 b)
  drawDoorWall(b);                           // 문이 붙은 오른쪽 측벽 (DOORWALL_VERTS 사다리꼴)

  // 방 꾸미기 오브제(랜덤) — 벽(상)·세우는 가구(중)는 문/침대보다 먼저(뒤에) 깔아 전경 오브젝트가 덮게.
  drawWallDecor(seamY);                      // 상: 벽에 거는 것 (시계·초상화·전단지…)
  drawStandingDecor(seamY, floorY);          // 중: 바닥에 세우는 가구 (장롱·책상·서랍…)

  // 문(우측 탈출구) → 침대(좌측, 매몰 존). 침대가 캐릭터 앞쪽이라 나중에 그림.
  drawDoor(doorCx, floorY, doorTopY);
  drawBed(6 * S, floorY + 8 * S);   // 침대 다리를 전경 바닥선(캐릭터 발높이)에 맞춤

  drawFloorClutter(seamY, floorY);  // 하: 바닥 잡동사니 (박스·쓰레기봉투…) — 캐릭터보다 뒤에 깔아 가독성 유지
...
// ---------- 방 꾸미기 오브제 (랜덤 배치) ----------
// 매 플레이마다 상(벽)/중(가구)/하(바닥)에서 몇 개를 골라 위치·깊이를 정한다 → 판마다 다른 방.
// 반환 배열의 각 항목: { tier, idx(=DECOR_DEFS[tier] 인덱스), x(0~1 가로), depth(0~1 앞뒤) }.
// 같은 시스템을 아파트뷰 이웃 방 생성에도 재사용(buildApartment).
// 중 가구 중 '항상' 나오는 것 (책상·장롱) — 파일명으로 인덱스를 찾아 배열 순서가 바뀌어도 안전.
const MID_ALWAYS_FILES = ['책상.png', '장롱.png'];
function midAlwaysIdxs() {
  return MID_ALWAYS_FILES.map(f => DECOR_DEFS.mid.findIndex(d => d.file === f)).filter(i => i >= 0);
}

function makeRoomDecor() {
  if (!IMAGES.decor) return [];   // preload 전 안전장치
  const decor = [];
  // 침대(좌)·문(우)을 피해 슬롯 배치. 벽(상)은 좀 더 넓게, 가구/바닥은 가운데로.
  addDecorTier(decor, 'high', irnd(1, 2), [0.18, 0.32, 0.46, 0.60, 0.74]);
  // 중: 책상·장롱은 항상 + 나머지에서 0~1개 더 (방마다 변화)
  const forcedMid = midAlwaysIdxs();
  addDecorTier(decor, 'mid', forcedMid.length + irnd(0, 1), [0.34, 0.50, 0.64, 0.77], forcedMid);
  addDecorTier(decor, 'low',  irnd(1, 2), [0.30, 0.45, 0.60, 0.74]);
  return decor;
}

// 정수 난수 [a, b] (양끝 포함)
function irnd(a, b) { return Math.floor(random(a, b + 1)); }

// 한 계층(tier)에서 count개를 서로 다른 에셋·슬롯으로 골라 decor에 추가.
// forced(인덱스 배열)는 항상 먼저 포함하고, 나머지를 랜덤으로 채운다.
function addDecorTier(decor, tier, count, slots, forced) {
  const defs = IMAGES.decor[tier];
  if (!defs || !defs.length) return;
  count = Math.min(count, defs.length, slots.length);
  forced = (forced || []).filter(i => i >= 0 && i < defs.length).slice(0, count);
  const rest = shuffle([...defs.keys()].filter(i => !forced.includes(i)));  // 강제에 없는 나머지
  const idxs = forced.concat(rest).slice(0, count);          // 강제 먼저 + 랜덤 채움
  const slotIdx = shuffle([...slots.keys()]).slice(0, count); // 서로 다른 가로 슬롯
  for (let k = 0; k < count; k++) {
    decor.push({
      tier,
      idx: idxs[k],
      x: constrain(slots[slotIdx[k]] + random(-0.025, 0.025), 0.04, 0.96),
      depth: random(0, 1),
    });
  }
}

// 한 오브제 그리기 공통 — 중심 cx·바닥 baseY 기준으로 종횡비 유지해 그림. shadow면 발치 접지 타원.
function drawDecorImg(d, cx, baseY, shadow) {
  if (!d || !d.img) return;
  const h = d.h * S;
  const w = h * (d.img.width / d.img.height);
  if (shadow) { noStroke(); fill(0, 52); ellipse(cx, baseY - 1 * S, w * 0.80, h * 0.12); }
  push();
  imageMode(CENTER);
  noTint();
  image(d.img, cx, baseY - h / 2, w, h);
  pop();
}

// 상 — 벽에 거는 오브제. 벽면(ROOM_TOP~seamY)의 중상단에 깊이로 높이를 살짝 달리해 건다.
function drawWallDecor(seamY) {
  for (const it of (roomDecor || [])) {
    if (it.tier !== 'high') continue;
    const cx = lerp(PLAYER_RANGE_LEFT, PLAYER_RANGE_RIGHT, it.x);
    const cy = lerp(ROOM_TOP, seamY, 0.30 + it.depth * 0.28);   // 벽 중상단(겹쳐도 자연스럽게)
    const d = IMAGES.decor.high[it.idx];
    if (!d || !d.img) continue;
    const h = d.h * S, w = h * (d.img.width / d.img.height);
    push(); imageMode(CENTER); noTint();
    image(d.img, cx, cy, w, h);
    pop();
  }
}

// 중 — 바닥에 세우는 가구. 뒷벽(seamY) 앞 바닥에 base를 두되 깊이로 앞뒤를 살짝 흩뿌림. 뒤→앞 순서로.
function drawStandingDecor(seamY, floorY) {
  const items = (roomDecor || []).filter(it => it.tier === 'mid').sort((a, b) => a.depth - b.depth);
  for (const it of items) {
    const cx = lerp(PLAYER_RANGE_LEFT, PLAYER_RANGE_RIGHT, it.x);
    const baseY = lerp(seamY + 6 * S, lerp(seamY, floorY, 0.5), it.depth);
    drawDecorImg(IMAGES.decor.mid[it.idx], cx, baseY, true);
  }
}

// 하 — 바닥에 널브러진 잡동사니. 가구보다 앞쪽(바닥 중앙) 깔되 캐릭터보다는 먼저 그려 가독성 유지.
// (이미 바닥에 붙어 있는 에셋이라 접지 그림자는 생략 — shadow=false)
function drawFloorClutter(seamY, floorY) {
  const items = (roomDecor || []).filter(it => it.tier === 'low').sort((a, b) => a.depth - b.depth);
  for (const it of items) {
    const cx = lerp(PLAYER_RANGE_LEFT, PLAYER_RANGE_RIGHT, it.x);
    const baseY = lerp(lerp(seamY, floorY, 0.4), floorY - 4 * S, it.depth);
    drawDecorImg(IMAGES.decor.low[it.idx], cx, baseY, false);
  }
}
...
// b(위치/방 밝기) → 방 밝기 비율 f(0~1). 뒷벽·문벽이 공유.
function roomBrightnessF(b) {
  return constrain(map(b, ROOM_DARK_AT, ROOM_BRIGHT_AT, ROOM_MIN_BRIGHT, ROOM_MAX_BRIGHT), 0, 1);
}

// 문이 붙어있는 오른쪽 '측벽' (참고 이미지) — 직사각형이 아닌 사다리꼴.
// ▼▼ 여기 4꼭짓점(화면폭 W·높이 H 비율, 0~1)을 직접 찍어 모양을 맞추면 됨. 시계방향: 좌상→우상→우하→좌하.
const DOORWALL_VERTS = [
  [0.9, 0.00],   // 0 좌상 (뒷벽과 만나는 코너, 위쪽)
  [1.00, 0.00],   // 1 우상 (화면 오른쪽 위 끝)
  [1.00, 0.88],   // 2 우하 (오른쪽 끝에서 바닥과 만나는 높이 — 낮게)
  [0.9, 0.55],   // 3 좌하 (코너에서 바닥과 만나는 높이 — 뒷벽 바닥선 근처)
];
// ▲▲
const DWALL_DARK = [52, 49, 60], DWALL_BRIGHT = [122, 116, 130];   // 측벽 색(어두운~중간 따뜻한 회색, 밝기 보간)
function drawDoorWall(b) {
  const f = roomBrightnessF(b);
  const V = DOORWALL_VERTS.map(([fx, fy]) => [fx * W, fy * H]);
  noStroke();
  // 측벽 면
  fill(lerp(DWALL_DARK[0], DWALL_BRIGHT[0], f), lerp(DWALL_DARK[1], DWALL_BRIGHT[1], f), lerp(DWALL_DARK[2], DWALL_BRIGHT[2], f));
  beginShape(); for (const [x, y] of V) vertex(x, y); endShape(CLOSE);
  // 방 색조 살짝만 (뒷벽보다 옅게 → 더 중성)
  const tintC = hueOverride || PLAYER_TINT;
  fill(tintC[0], tintC[1], tintC[2], 16);
  beginShape(); for (const [x, y] of V) vertex(x, y); endShape(CLOSE);
  // 경계선: 코너(좌상→좌하) + 발치 걸레받이(좌하→우하)
  stroke(0, 0, 0, 60); strokeWeight(2.5 * S);
  line(V[0][0], V[0][1], V[3][0], V[3][1]);
  stroke(0, 0, 0, 48);
  line(V[3][0], V[3][1], V[2][0], V[2][1]);
  noStroke();
}
...
// 문(우측 탈출구). 세로로 긴 PNG의 콘텐츠 bbox(빛 포함)만 잘라 바닥에 세움.
function drawDoor(centerX, floorY, topY) {
  const img = IMAGES.door;
  const h = floorY - topY;
  const w = h * DOOR_RATIO;            // 에셋 종횡비(빛 포함) 유지
  if (img) {
    noTint();
    image(img, centerX - w / 2, floorY - h, w, h, DOOR_SRC[0], DOOR_SRC[1], DOOR_SRC[2], DOOR_SRC[3]);
  } else {
...
function drawBoxStack(x, baseY, count) {
  ...
  const rackCx = x - RACK_DX_RATIO * w + 10;          // 캐릭터 중심에서 등(왼쪽)으로
  const bw = 46 * S;
  const bh = bw / BOX_RATIO;             // 납작한 슬랩 (box-최종 종횡비)
  const step = bh * 0.86;                // 슬랩이 차곡차곡 (살짝 겹쳐 쌓임)
  for (let i = 0; i < count; i++) {
    // 쌓일 때마다 아주 살짝 좌우로 엇갈리게(지그재그) + 미세하게 기울임 — 박스마다 폭을 조금씩 달리해 자연스럽게
    const sign = (i % 2 === 0) ? -1 : 1;
    const wob = 0.6 + 0.2 * Math.abs(Math.sin(i * 1.7));   // 0.6~1.0 (결정적, 흔들림 고정)
    const dx = sign * 3 * S * wob;
    const rot = sign * 0.03 * wob;
    const cy = rackTopY + 2 * S - bh / 2 - i * step;       // 박스 중심 y (맨 아래가 지게에)
    const flashing = i === topIdx && flash && Math.floor(millis() / 80) % 2 === 0;
    push();
    ...
    if (img) {
      ...
      noStroke(); fill(flashing ? 255 : 200, flashing ? 240 : 140, flashing ? 150 : 80);
...
// ---------- buttons (우측 하단, 이미지 에셋) ----------
// 참고: src/화면 UI/플레이화면 메인 UI+ 둘러보기 버튼.png
//  앞으로 가기 = 큰 피치 버튼(우측 끝) / 잠시 쉬기 = 작은 녹색 버튼(그 왼쪽) / ▶둘러보기 = 잠시쉬기 위 텍스트
// 에셋 콘텐츠 bbox(투명 여백 제외) + 종횡비 — 히트영역 ry를 이 비율로 산출해 보이는 버튼과 일치시킴.
const FWD_SRC = [40, 94, 2273, 1296],  FWD_RATIO = 2273 / 1296;   // 앞으로가기버튼.png
const REST_SRC = [14, 74, 1364, 713],  REST_RATIO = 1364 / 713;   // 잠시쉬기버튼 _4.png
function forwardBtn() {
  const w = W * 0.27;
  return { cx: W * 0.855, cy: H * 0.78, rx: w / 2, ry: (w / FWD_RATIO) / 2 };
}
function restBtn() {
  const w = W * 0.185;
  return { cx: W * 0.62, cy: H * 0.825, rx: w / 2, ry: (w / REST_RATIO) / 2 };
}
...
// 버튼 이미지를 히트영역(중심·반지름)에 맞춰 그림. pressed면 살짝 작게(누름 피드백).
function drawBtnImg(img, e, src, pressed) {
  if (!img) return;
  const k = pressed ? 0.95 : 1;
  push();
  imageMode(CENTER);
  noTint();
  image(img, e.cx, e.cy, e.rx * 2 * k, e.ry * 2 * k, src[0], src[1], src[2], src[3]);
  pop();
}

function drawButtons() {
  drawBtnImg(IMAGES.btnRest,    restBtn(),    REST_SRC, heldRest);             // 잠시 쉬기 (누르면 살짝 작게)
  drawBtnImg(IMAGES.btnForward, forwardBtn(), FWD_SRC,  millis() < walkUntil); // 앞으로 가기 (탭 직후 살짝 작게)
}
...
function handlePress(mx, my) {
  ...
  if (phase === 'lookaround') {
    // 줌아웃 + 손잡기 인트로 + 문구 노출이 끝난 뒤에만 되돌아가기 가능
    if (millis() - lookStart > ZOOM_MS + HAND_MS + LOOK_MSG_MS && inRect(mx, my, backBtn())) {
...
function lookBtn() {
  // ▶ 둘러보기 — 잠시 쉬기 버튼 위 텍스트 버튼 (타원 히트영역, 잠시쉬기와 같은 x에 정렬)
  return { cx: W * 0.60, cy: H * 0.66, rx: W * 0.085, ry: H * 0.055 };
}
...
function returnFromLookaround() {
  boxCount = Math.round(boxCount * LOOK_BOX_RELIEF);  // 박스 80% 덜어냄 (구제)
  lastBoxDrop = millis();                             // 박스 타이머 리셋 (즉시 드랍 방지)
  phase = 'zoomin';                                   // 내 방으로 줌인 → 끝나면 'playing'
  zoomStart = millis();
}
...
function buildApartment() {
  apartmentRooms = [];
  // 0번 = 나 (현재 실제 상태 반영) — 색조는 테마 네이비 #04216C, 꾸밈은 내 방과 동일
  apartmentRooms.push({
    isPlayer: true,
    ...
    decor: roomDecor,              // 줌아웃/줌인 시 내 방이 그대로 보이도록 같은 배치 공유
  });
  // 나머지 = 이웃: 같은 파랑 계열에서 색조·채도만 살짝 달리 (알록달록하지 않게) + 방마다 다른 오브제 배치
  ...
  for (let i = 1; i < N; i++) {
    ...
    apartmentRooms.push({
      ...
      decor: makeRoomDecor(),      // 이웃 방도 같은 랜덤 시스템으로 — 집집마다 다른 방
    });
  }
}
...
// 아파트 그리드 기하(창 크기·위치) — W/H에서 계산. drawApartment와 줌 전환이 공유.
function apartmentLayout() {
  const bx = W * 0.10, bw = W * 0.80, byTop = H * 0.05;
  const cols = 3, rows = Math.ceil(APARTMENT_ROOMS / cols);
  ...
  return { bx, bw, byTop, cols, rows, cw, ch, gapX, gapY, gx0, gy0 };
}

// 그리드 i번 창의 사각형 (줌 전환 목표/시작 위치)
function apartmentCellRect(i) {
  const L = apartmentLayout();
  return {
    x: L.gx0 + (i % L.cols) * (L.cw + L.gapX),
    y: L.gy0 + Math.floor(i / L.cols) * (L.ch + L.gapY),
    w: L.cw, h: L.ch,
  };
}

// 줌 애니메이션에 쓸 '현재 내 상태' 방 데이터 (메인 플레이와 동일하게 보이도록)
function liveRoom() {
  return {
    isPlayer: true,
    prog: constrain(playerX, 0.06, 0.94),
...
function drawApartment() {
  const L = apartmentLayout();
  const { bx, bw, byTop, cw, ch, gapX, gx0, gy0, cols } = L;
  ...
  for (let i = 0; i < apartmentRooms.length; i++) {
    const r = apartmentCellRect(i);
    drawApartmentWindow(r.x, r.y, cw, ch, apartmentRooms[i]);
  }
}

// 되돌아가기 → 내 방(0번 창)으로 줌인하며 메인 플레이로 복귀하는 애니메이션
function drawZoomIn() {
  const t = Math.min((millis() - zoomStart) / ZOOM_MS, 1);
  const te = easeInOut(t);
  drawApartment();                               // 뒤에 아파트(점점 가려짐)
  const c = apartmentCellRect(0);                // 내 방 창
  drawMiniPlayScene(lerp(c.x, 0, te), lerp(c.y, 0, te), lerp(c.w, W, te), lerp(c.h, H, te), liveRoom());
  if (t >= 1) phase = 'playing';                 // 도착하면 메인 플레이 재개
}
...
function drawMiniPlayScene(px, py, pw, ph, room) {
  const sv = {
    ...
    playerX, boxCount, boxFlashAt, heldRest, walkUntil, brightnessOverride, hueOverride, roomDecor,
  };
  push();
  ...
  roomDecor = room.decor || [];           // 방별 오브제 배치

  drawScene();                            // 방 배경(밝기+색조)+오브제+문+침대+캐릭터+박스
...
function drawLookaroundOverlay() {
  const elapsed = millis() - lookStart - ZOOM_MS - HAND_MS;   // 줌아웃 + 손잡기 인트로 이후 기준
