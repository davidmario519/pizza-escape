...
function drawScene() {
  ...
  drawRoomBackground(seamY, b);          // 문 왼쪽 하단 기준 뒷벽/바닥 수평 분할 (참고 배경 어두운→밝은)
...
const PAL_DARK_WALL  = [57, 57, 82],    PAL_BRIGHT_WALL  = [206, 206, 221];
const PAL_DARK_FLOOR = [136, 136, 160], PAL_BRIGHT_FLOOR = [222, 222, 234];
...
function drawRoomBackground(seamY, b) {
  // 가로 구배 밝기 비율: 왼쪽 끝(fLeft) ~ 오른쪽 끝(fRight). 진행도 b로 양끝을 들어올림.
  const fLeft  = constrain(0.04 + 0.34 * b, 0, 1);
  const fRight = constrain(0.62 + 0.38 * b, 0, 1);
  const gx1 = PANEL_LEFT;   // 구배 오른쪽 끝 = 플레이 영역 우측(문=빛)
...
function drawVignette(b) {
  // 왼쪽 그늘 (좌→우 페이드, 오른쪽 절반은 건드리지 않음)
  const leftDark = (1 - b) * 0.50;   // 0(문) ~ 0.50(침대)
