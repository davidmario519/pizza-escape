...
const APARTMENT_ROOMS = 9;         // 아파트뷰 창문 개수 (3열 × 3행, 0번=플레이어 방)
const LOOK_MESSAGES = [
  '다들 비슷하구나.\n나도 힘내야지. 재도전!',
  '괜찮아. 잠시 쉴수도 있지.\n다시 해보자.',
  '내가 침대로 갔던건 추진력을\n얻기 위함 이었다.',
...
function drawScene() {
  ...
  drawRoomBackground(seamY, b, doorCx);  // 문 왼쪽 하단 기준 뒷벽/바닥 분할 + 밝기 최대점=문(doorCx)
  ...
  drawVignette(b);            // 가장자리 음영 — 시작 시 짙고 탈출선에 다가갈수록 옅게 (고립감+깊이)
}
...
//   플레이어가 오른쪽으로 갈수록 더 밝은 영역으로 들어가고(위치 기반 동적),
//   진행도 b(=playerX)로 양 끝 밝기 자체도 들어올려 문에 다가갈수록 방 전체가 밝아진다.
function drawRoomBackground(seamY, b, brightX) {
  // 가로 구배 밝기 비율: 왼쪽 끝(fLeft) ~ 오른쪽 끝(fRight). 좌우 폭을 크게 벌려 대비를 또렷하게.
  // (왼쪽은 진행해도 어둡게 유지, 오른쪽=문은 항상 거의 최대 밝기 → 한 화면에서도 명암차 분명)
  const fLeft  = constrain(0.00 + 0.16 * b, 0, 1);   // 0.00 ~ 0.16 (침대 쪽은 계속 어둡게)
  const fRight = constrain(0.86 + 0.14 * b, 0, 1);   // 0.86 ~ 1.00 (문 쪽은 항상 환하게)
  const fMid   = lerp(fLeft, fRight, 0.30);          // 중간은 어두운 쪽에 가깝게 → 좌측 어둠을 넓게, 빛은 문에 집중
  // 밝기 최대점 = 문(brightX). 버튼 패널 영역은 제외(패널 뒤가 아니라 문에서 가장 밝게).
  const gx1 = brightX || PANEL_LEFT;

  noStroke();
  // 뒷벽 (가로 밝기 구배: 0=왼쪽 어둠 / 0.5=중간(어두운 쪽) / 1=문 환함)
  const wg = drawingContext.createLinearGradient(0, 0, gx1, 0);
  wg.addColorStop(0,   mixRGB(PAL_DARK_WALL, PAL_BRIGHT_WALL, fLeft));
  wg.addColorStop(0.5, mixRGB(PAL_DARK_WALL, PAL_BRIGHT_WALL, fMid));
  wg.addColorStop(1,   mixRGB(PAL_DARK_WALL, PAL_BRIGHT_WALL, fRight));
  drawingContext.fillStyle = wg;
  drawingContext.fillRect(0, 0, W, seamY);
  // 바닥 (동일 구배)
  const flg = drawingContext.createLinearGradient(0, 0, gx1, 0);
  flg.addColorStop(0,   mixRGB(PAL_DARK_FLOOR, PAL_BRIGHT_FLOOR, fLeft));
  flg.addColorStop(0.5, mixRGB(PAL_DARK_FLOOR, PAL_BRIGHT_FLOOR, fMid));
  flg.addColorStop(1,   mixRGB(PAL_DARK_FLOOR, PAL_BRIGHT_FLOOR, fRight));
  drawingContext.fillStyle = flg;
  ...
  // 바닥 원근 그라데이션(세로) — 경계(안쪽) 살짝 어둡게, 앞쪽 살짝 밝게
...
// 조명 보강 — 왼쪽(침대) 그늘로 좌측을 더 어둡게(문 쪽은 손대지 않아 우측은 밝게 유지).
// 진행도 b로 그늘이 옅어져, 문에 다가갈수록 방 전체가 환해진다. + 위/아래 가장자리 옅은 프레이밍.
function drawVignette(b) {
  // 왼쪽 그늘 (좌→우 페이드, 오른쪽 절반은 건드리지 않음) — 좌측을 더 깊게 눌러 대비 강화
  const leftDark = (1 - b) * 0.52 + 0.05;   // 0.05(문) ~ 0.57(침대)
  const lg = drawingContext.createLinearGradient(0, 0, W * 0.52, 0);
  lg.addColorStop(0, 'rgba(0,0,0,' + leftDark.toFixed(3) + ')');
  lg.addColorStop(1, 'rgba(0,0,0,0)');
  drawingContext.fillStyle = lg;
  drawingContext.fillRect(0, 0, W * 0.52, H);

  // 위/아래 가장자리 옅은 음영 (프레이밍, 우측 밝기에는 영향 적게)
  const eg = drawingContext.createLinearGradient(0, 0, 0, H);
  eg.addColorStop(0,    'rgba(0,0,0,0.20)');
...
// ---------- HUD ----------
function drawHUD() {
  // 지게 무게 — 우측 상단 (밝은 우측 벽 위라 어두운 글자로 대비 확보)
  textAlign(RIGHT, TOP);
  textSize(14 * S);
  fill(40, 44, 64, 215);
...
function drawApartment() {
  ...
  // 창문 그리드 (3열 × 3행, 16:9 창) — '되돌아가기' 버튼 위 영역에 맞춰 크기 산출.
  const cols = 3;
  const rows = Math.ceil(APARTMENT_ROOMS / cols);
  const gridTop = byTop, gridBottom = H - 92 * S;  // 하단은 되돌아가기 버튼 위로
  const gridH = gridBottom - gridTop;
  const gapY = gridH * 0.055;
  ...
  for (let i = 0; i < apartmentRooms.length; i++) {
    ...
    const x = gx0 + c * (cw + gapX);
    const y = gy0 + r * (ch + gapY);
    if (r < rows - 1 && i % 2 === 0) drawBrick(x + cw * 0.6, y + ch + gapY * 0.34, S);  // 창 사이 벽돌 장식
  ...
  text('다들 각자의 방에서 버티고 있다', W / 2, H * 0.013);
  textAlign(CENTER, CENTER);
}

// 좌우 하늘 + 상단 모서리 픽셀 구름 (입장 시 고정 위치)
function drawClouds(bx, bw, byTop) {
  pixelCloud(bx * 0.28,           H * 0.18, 2.0 * S);
  pixelCloud(bx * 0.40,           H * 0.58, 1.5 * S);
  pixelCloud(bx + bw + bx * 0.16, H * 0.28, 2.1 * S);
  pixelCloud(bx + bw + bx * 0.30, H * 0.66, 1.4 * S);
  pixelCloud(W * 0.32,            byTop * 0.34, 1.3 * S);
}

function pixelCloud(cx, cy, s) {
  noStroke();
  fill(255, 255, 255, 232);
  rect(cx,          cy,         26 * s, 10 * s, 2 * s);
  rect(cx + 7 * s,  cy - 7 * s, 15 * s, 12 * s, 2 * s);
  rect(cx - 6 * s,  cy + 3 * s, 12 * s,  8 * s, 2 * s);
  rect(cx + 19 * s, cy + 2 * s, 12 * s,  8 * s, 2 * s);
}

// 작은 회색 계단형 블록 (참고 UI의 창 사이 장식)
function drawBrick(x, y, s) {
  noStroke();
  const u = 7 * s;
  fill(150, 151, 156);
  rect(x, y, u, u); rect(x + u, y, u, u);
  rect(x + u, y - u, u, u); rect(x + 2 * u, y - u, u, u);
  fill(0, 0, 0, 22);
  rect(x, y + u - 2 * s, 3 * u, 2 * s);
}
