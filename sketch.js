...
// 쌓이는 피자박스 에셋(피자박스-real-cut.png, 배경 투명처리)의 콘텐츠 bbox (검정 배경 제거 후)
const BOX_X = 199, BOX_Y = 203, BOX_CW = 1365, BOX_CH = 465;   // 종횡비 ≈ 2.935
...
function preload() {
  ...
  IMAGES = {
    ...
    // 게임 오브젝트 에셋 (디자인 PNG)
    box:  loadImage('src/게임 오브젝트/피자박스-real-cut.png'),                // 쌓이는 피자박스 (배경 투명처리, 콘텐츠 bbox 199,203 1365x465)
...
function drawBoxStack(x, baseY, count) {
  ...
  const bw = 40 *  S;
  const bh = bw * (BOX_CH / BOX_CW);    // 새 에셋 콘텐츠 종횡비
  const step = bh * 0.46;               // 앞면이 겹쳐 쌓이는 더미처럼
  for (let i = 0; i < count; i++) {
    const bx = rackCx - bw / 2;
    const by = rackTopY + 2 * S - bh - i * step;  // 맨 아래 박스가 지게에 얹히게 (+2 살짝 겹침)
    // 새 박스가 떨어진 직후 맨 위 박스를 밝게 반짝 (에셋이 사각형이 아니라 tint로)
    const flashing = i === topIdx && flash && Math.floor(millis() / 80) % 2 === 0;
    if (img) {
      if (flashing) tint(255, 255, 170); else noTint();
      image(img, bx, by, bw, bh, BOX_X, BOX_Y, BOX_CW, BOX_CH);   // 콘텐츠 bbox만
      noTint();
    } else {
