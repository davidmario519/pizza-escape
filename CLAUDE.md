...
**현재 코드와의 대응.** 코어 루프 + 둘러보기 + **엔딩 3갈래 + 현황·크레딧 리포트(part 3)**까지 구현됨. `phase` 종료 상태는 위 엔딩에 다음과 같이 대응한다(구현 완료):
...
- "본편" 기능 = **[둘러보기] → 줌아웃(아파트, 연대) → 플레이 복귀(박스 감소)** 흐름까지 코드에 들어옴. 엔딩 메시지 탭 → **part 3 리포트(DOM 오버레이)**(현황 통계 → 그래프 → 프로젝트 설명 → 인용 → 크레딧 → 피자 이스케이프 로고 + 리플레이) → 리플레이 시 `resetGame()`로 재도전(카운트다운부터).
...
- **엔딩 3갈래** (`drawEndCard()`). 피그마 `ending` 프레임(노드 `6:646`) 기반. 멈춘 플레이 씬 위에 톤별 워시 + 좌측정렬 메시지: **혼자탈출**(win·둘러보기 미사용=밝은 톤) / **진엔딩**(win·둘러보기 사용=파란 톤·연대) / **게임오버**(lose, `loseReason` `'bed'`·`'crushed'`로 멘트만 분기=어두운 톤). 멘트는 게임 스크립트 독스(Google Docs)에서 가져옴. 제목=`FONTS.macho`, 본문=`FONTS.nexon`.
- **현황·크레딧 리포트 (part 3)** — **캔버스가 아닌 `index.html`의 `#report` DOM 오버레이**(`style.css`로 스타일, p5는 `showReport()`/`replayFromReport()`로 토글). 엔딩 메시지 탭 → 오버레이가 캔버스를 덮으며 `noLoop()` → 현황 수치(34%) → 막대그래프(`#report-chart`, `setupReport()`가 `LONELINESS_DATA`로 렌더) → 프로젝트 설명 → 인용 3건(링크) → 크레딧 → `로고.png` + [게임 리플레이하기]. 리플레이 클릭 → 오버레이 숨김 + 캔버스 복원 + `resetGame()` + `loop()`. 네이비 테마/폰트는 `@font-face`(`EFMacho`/`NexonGothic`)로 게임과 공유.
...
- **리포트 실제 데이터/에셋** — 그래프 `LONELINESS_DATA`는 **예시(placeholder) 수치** → 실제 KOSIS 값으로 교체 필요. 크레딧은 텍스트만(서강대 AXT/Kakao Impact/안무서운회사 로고 이미지 미적용).
...
**Phase state machine.** A single `phase` string (`'title'` → `'countdown'` → `'playing'` → `'win'` | `'lose'`, with `'lookaround'` branching off `'playing'`) drives the whole game. `draw()` is the dispatcher: it short-circuits to the opening (`'title'`) or the rotate-hint when held portrait; otherwise it calls the matching `step*()` updater, renders the scene/HUD, then layers phase-specific overlays. State transitions happen *inside* the `step*()` functions (e.g. `stepPlaying()` flips `phase` to `'win'`/`'lose'`). A tap on an end card (after a ~900ms guard) calls `showReport()` — which reveals the DOM report overlay (part 3) and `noLoop()`s the canvas. `setup()` enters `'title'`; `resetGame()` restores all mutable state (and resets `textFont`) — replay skips the title, going straight to countdown.

**Endings (`drawEndCard()`) + report (part 3).** Three ending branches keyed off `phase`/`usedLookAround`/`loseReason`, content in `endingContent()`: 혼자탈출 (`win` & !`usedLookAround`, light tone), 진엔딩 (`win` & `usedLookAround`, blue tone), 게임오버 (`lose`, dark tone, `loseReason` picks the copy). Each draws the frozen play scene + a tonal full-screen wash + a left-aligned title (`FONTS.macho`) / body (`FONTS.nexon` = NEXON Lv1 Gothic), auto-shrunk to fit width. Ported from the Figma `ending` frame (node `6:646`); copy from the game-script Google Doc. A tap then opens the **part-3 report**: not canvas but a DOM overlay (`#report` in `index.html`, styled in `style.css`). `showReport()` adds `.show`, hides `canvasEl`, and `noLoop()`s; `setupReport()` (called once in `setup()`) renders the bar chart from `LONELINESS_DATA` into `#report-chart` and binds the replay button to `replayFromReport()` (hide overlay → `canvasEl.show()` → `resetGame()` → `loop()`). The overlay shares the navy theme via `@font-face` (`EFMacho`/`NexonGothic`).
...
**Fonts & images.** `preload()` loads `FONTS` (KBL Jump family + `macho` = EF_MACHO + `galmuri` = Galmuri11 + `nexon` = NEXON Lv1 Gothic) from `src/폰트/…` and `IMAGES` (e.g. `IMAGES.logo`) from `src/화면 UI/…`. Opening uses `FONTS.macho` (Korean headings/labels/button), `FONTS.kblExtended` ("HOW TO PLAY"), `FONTS.galmuri` (body); endings use `FONTS.macho` (title) + `FONTS.nexon` (body). The DOM report (part 3) loads the same EF_MACHO/NEXON via `@font-face` in `style.css` (`EFMacho`/`NexonGothic`). `loadFont`/`loadImage` are fetch-based (fonts: TTF only, no woff2) so the page **must be served over http** (Live Server / `python3 -m http.server`), not opened as `file://`. `startGame()` resets `textFont('sans-serif')` so gameplay text keeps the default font — gameplay art is still code-drawn and unfonted.
